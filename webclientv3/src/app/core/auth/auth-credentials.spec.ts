...
describe("AuthCredentials", () => {
  ...
  describe("#toHeaders()", () => {
    it("converts to a Headers containing ALL the required properties", () => {
      ...
      let headers = cred.toHeaders();
      let expected = new HttpHeaders(source);
      expect(headers.get("access-token")).toEqual("a");
      expect(headers.get("client")).toEqual("c");
      expect(headers.get("expiry")).toEqual("e");
      expect(headers.get("token-type")).toEqual("t");
      expect(headers.get("uid")).toEqual("u");
    });
  });
  ...
  describe("#saveToLocalStorage()", () => {
    it("saves the auth data to local storage", () => {
      ...
      expect(localStorage.getItem("access-token")).toEqual(
        source["access-token"]
      );
