...
describe('ScotiaImporterService', () => {
  ...
  describe('#convertToTransactions', () => {
    ...
    it('returns an empty list by default', () => {
      expect(scotiaImporter.convertToTransactions(input, startDate, endDate)).toEqual([]);
    });
    ...
    describe('when handling a normal POS transaction', () => {
      ...
      xit('returns a transaction list with one transaction', () => {
        input = '01/23/2015		\n' +
          ...
          'PORT OF SPAINTT\n' +
          '$0.75 TTD	\n';
        let result = scotiaImporter.convertToTransactions(input, startDate, endDate, 'bank-account');
      ...
      xit('handles service charge transactions', () => () => {
        input = '02/23/2015		\n' +
          ...
          '$4.00 TTD		\n' +
          '02/23/2015	\n';
        let result = scotiaImporter.convertToTransactions(input, startDate, endDate);
        expect(result.length).toEqual(1);
