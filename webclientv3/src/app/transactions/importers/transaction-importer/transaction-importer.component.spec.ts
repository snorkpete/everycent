...
import {TRANSACTION_IMPORTER_PROVIDERS, TransactionImporterModule} from "../transaction-importer.module";
...
xdescribe('TransactionImporterComponent', () => {
  ...
  it('clicking the import button causes import function to be triggered', () => {
    ...
    let importerService: TransactionImporterService = TestBed.get(TransactionImporterService);
    let spy = spyOn(importerService, "convertToTransactions").and.returnValue(sampleTransactions);
  ...
  xit('contains a bank import type selector', () => {
    ...
    expect(options[1].textContent).toContain('ABN Amro Credit Card (not implemented)');
    expect(options[2].textContent).toContain('Scotia Bank Account');
    expect(options[3].textContent).toContain('FCB Bank Account');
    expect(options[4].textContent).toContain('FCB Credit Card (not implemented)');
