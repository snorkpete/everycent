angular.module('everycent').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/budgets/ec-allocation-list-editor-directive.html',
    "<div class=row><div class=col-xs-12><ec-panel title=Allocations type=primary><table class=\"table table-bordered rounded\"><thead><tr class=heading><th>Name</th><th ng-show=\"\">Category</th><th class=text-right>Amount</th><th class=text-right>Standing Order?</th><th>Bank Account</th><th>Actions</th></tr></thead><tbody><tr class=category-heading ng-repeat-start=\"category in vm.groupedAllocationCategories\"><td>{{ category.name }}</td><td ng-show=\"\"></td><td class=text-right>{{ vm.util.total(category.allocations, 'amount') | ecToDollars }}</td><td></td><td></td><td></td></tr><tr class=category-detail ec-allocation-list-row ng-repeat=\"allocation in category.allocations\" allocation=allocation is-edit-mode=vm.isEditMode></tr><tr class=category-action ng-repeat-end><td colspan=6 ng-show=vm.isEditMode><button class=\"btn btn-default\" ng-click=vm.addNewAllocationInCategory(category)>Add {{category.name}} Allocation</button></td></tr></tbody><tfoot><tr class=total><th>Total</th><th ng-show=\"\"></th><th class=text-right>{{ vm.util.total(vm.budget.allocations, 'amount') | ecToDollars }}</th><th></th><th></th><th></th></tr></tfoot></table><div class=row><div class=col-xs-2 ng-hide=vm.isEditMode><button class=\"btn btn-primary\" ng-click=vm.switchToEditMode()>Make Changes</button></div><div class=col-xs-9><div class=pull-right ng-show=vm.isEditMode><button class=\"btn btn-success\" ng-click=vm.switchToViewMode()>Back to View Mode</button> &nbsp; &nbsp; <button class=\"btn btn-danger\" ng-click=vm.cancelEdit()>Cancel</button></div></div></div></ec-panel></div></div><br>"
  );


  $templateCache.put('app/budgets/ec-allocation-list-row-directive.html',
    "<tr ng-class=\"{ danger: vm.allocation.deleted }\"><td><input ng-show=vm.isEditMode ng-model=\"vm.allocation.name\"> <span ng-hide=vm.isEditMode ng-bind=vm.allocation.name></span></td><td ng-show=\"\"><select ng-show=vm.isEditMode ng-model=vm.allocation.allocation_category ng-change=\"vm.ref.updateReferenceId(vm.allocation, 'allocation_category')\" ng-options=\"category.name for category in vm.allocationCategories track by category.id\"></select><span ng-hide=vm.isEditMode ng-bind=vm.allocation.allocation_category.name></span></td><td class=text-right><input type=number ng-show=vm.isEditMode ng-model=vm.allocation.amount ec-as-dollars> <span ng-hide=vm.isEditMode ng-bind=\"vm.allocation.amount | ecToDollars \"></span></td><td class=text-right><input type=checkbox ng-show=vm.isEditMode ng-model=\"vm.allocation.is_standing_order\"> <span ng-hide=vm.isEditMode ng-bind=\"vm.allocation.is_standing_order ? 'Yes' : 'No'\"></span></td><td><span ng-show=vm.allocation.is_standing_order><select ng-show=vm.isEditMode ng-model=vm.allocation.bank_account ng-change=\"vm.ref.updateReferenceId(vm.allocation, 'bank_account')\" ng-options=\"bankAccount.name for bankAccount in vm.bankAccounts track by bankAccount.id\"></select><span ng-hide=vm.isEditMode ng-bind=vm.allocation.bank_account.name></span></span></td><td><span ng-hide=vm.allocation.deleted ng-click=\"vm.markForDeletion(vm.allocation, true)\" class=\"text-danger pull-right pointer\"><ec-icon type=trash></ec-icon></span> <span ng-show=vm.allocation.deleted ng-click=\"vm.markForDeletion(vm.allocation, false)\" class=\"text-success pull-right pointer\"><ec-icon type=refresh></ec-icon></span></td></tr>"
  );


  $templateCache.put('app/budgets/ec-income-list-editor-directive.html',
    "<div class=row><div class=col-xs-12><ec-panel title=Incomes type=primary><table class=\"table table-bordered rounded\"><thead><tr class=heading><th>Name</th><th>Bank Account</th><th class=text-right>Amount</th><th>Actions</th></tr></thead><tbody><tr ng-repeat=\"income in vm.budget.incomes\" ng-class=\"{ danger: income.deleted }\"><td><input ng-show=vm.isEditMode ng-model=\"income.name\"> <span ng-hide=vm.isEditMode ng-bind=income.name></span></td><td><select ng-show=vm.isEditMode ng-model=income.bank_account ng-change=\"vm.ref.updateReferenceId(income, 'bank_account')\" ng-options=\"bankAccount.name for bankAccount in vm.bankAccounts track by bankAccount.id\"></select><span ng-hide=vm.isEditMode ng-bind=income.bank_account.name></span></td><td class=text-right><input type=number ng-show=vm.isEditMode ng-model=income.amount ec-as-dollars> <span ng-hide=vm.isEditMode ng-bind=\"income.amount | ecToDollars\"></span></td><td><span ng-hide=income.deleted ng-click=\"vm.markForDeletion(income, true)\" class=\"text-danger pull-right pointer\"><ec-icon type=trash></ec-icon></span> <span ng-show=income.deleted ng-click=\"vm.markForDeletion(income, false)\" class=\"text-success pull-right pointer\"><ec-icon type=refresh></ec-icon></span></td></tr></tbody><tfoot><tr class=total><th>Total</th><th></th><th class=text-right>{{ vm.util.total(vm.budget.incomes, 'amount') | ecToDollars }}</th><th></th></tr></tfoot></table><div class=row><div class=col-xs-2 ng-hide=vm.isEditMode><button class=\"btn btn-primary\" ng-click=vm.switchToEditMode()>Make Changes</button></div><div class=col-xs-3 ng-show=vm.isEditMode><button class=\"btn btn-primary\" ng-click=vm.addNewIncome()>Add Income</button></div><div class=col-xs-9><div class=pull-right ng-show=vm.isEditMode><button class=\"btn btn-success\" ng-click=vm.switchToViewMode()>Back to View Mode</button> &nbsp; &nbsp; <button class=\"btn btn-danger\" ng-click=vm.cancelEdit()>Cancel</button></div></div></div></ec-panel></div></div><br>"
  );
