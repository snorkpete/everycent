angular.module('everycent').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/budgets/ec-allocation-list-editor-directive.html',
    "<div class=row><div class=col-xs-12><ec-panel title=Allocations type=primary><table class=\"table table-bordered rounded\"><thead><tr class=heading><th>Name</th><th>Category</th><th>Amount</th><th>Standing Order?</th><th>Bank Account</th><th>Actions</th></tr></thead><tbody><tr ng-repeat=\"allocation in vm.budget.allocations\" ng-class=\"{ danger: allocation.deleted }\"><td><input ng-show=vm.isEditMode ng-model=\"allocation.name\"> <span ng-hide=vm.isEditMode ng-bind=allocation.name></span></td><td><select ng-show=vm.isEditMode ng-model=allocation.allocation_category ng-change=\"vm.ref.updateReferenceId(allocation, 'allocation_category')\" ng-options=\"category.name for category in vm.allocationCategories track by category.id\"></select><span ng-hide=vm.isEditMode ng-bind=allocation.allocation_category.name></span></td><td><input type=number ng-show=vm.isEditMode ng-model=allocation.amount ec-as-dollars> <span ng-hide=vm.isEditMode ng-bind=\"allocation.amount | ecToDollars \"></span></td><td><input type=checkbox ng-show=vm.isEditMode ng-model=\"allocation.is_standing_order\"> <span ng-hide=vm.isEditMode ng-bind=\"allocation.is_standing_order ? 'Yes' : 'No'\"></span></td><td><span ng-show=allocation.is_standing_order><select ng-show=vm.isEditMode ng-model=allocation.bank_account ng-change=\"vm.ref.updateReferenceId(allocation, 'bank_account')\" ng-options=\"bankAccount.name for bankAccount in vm.bankAccounts track by bankAccount.id\"></select><span ng-hide=vm.isEditMode ng-bind=allocation.bank_account.name></span></span></td><td><span ng-hide=allocation.deleted ng-click=\"vm.markForDeletion(allocation, true)\" class=\"text-danger pull-right pointer\"><ec-icon type=trash></ec-icon></span> <span ng-show=allocation.deleted ng-click=\"vm.markForDeletion(allocation, false)\" class=\"text-success pull-right pointer\"><ec-icon type=refresh></ec-icon></span></td></tr></tbody><tfoot><tr class=total><th>Total</th><th></th><th>{{ vm.util.total(vm.budget.allocations, 'amount') | ecToDollars }}</th><th></th><th></th></tr></tfoot></table><div class=row><div class=col-xs-2 ng-hide=vm.isEditMode><button class=\"btn btn-primary\" ng-click=vm.switchToEditMode()>Make Changes</button></div><div class=col-xs-3 ng-show=vm.isEditMode><button class=\"btn btn-primary\" ng-click=vm.addNewAllocation()>Add Allocation</button></div><div class=col-xs-9><div class=pull-right ng-show=vm.isEditMode><button class=\"btn btn-success\" ng-click=vm.switchToViewMode()>Back to View Mode</button> &nbsp; &nbsp; <button class=\"btn btn-danger\" ng-click=vm.cancelEdit()>Cancel</button></div></div></div></ec-panel></div></div><br>"
  );


  $templateCache.put('app/budgets/ec-income-list-editor-directive.html',
    "<div class=row><div class=col-xs-12><ec-panel title=Incomes type=primary><table class=\"table table-bordered rounded\"><thead><tr class=heading><th>Name</th><th>Bank Account</th><th>Amount</th><th>Actions</th></tr></thead><tbody><tr ng-repeat=\"income in vm.budget.incomes\" ng-class=\"{ danger: income.deleted }\"><td><input ng-show=vm.isEditMode ng-model=\"income.name\"> <span ng-hide=vm.isEditMode ng-bind=income.name></span></td><td><select ng-show=vm.isEditMode ng-model=income.bank_account ng-change=\"vm.ref.updateReferenceId(income, 'bank_account')\" ng-options=\"bankAccount.name for bankAccount in vm.bankAccounts track by bankAccount.id\"></select><span ng-hide=vm.isEditMode ng-bind=income.bank_account.name></span></td><td><input type=number ng-show=vm.isEditMode ng-model=income.amount ec-as-dollars> <span ng-hide=vm.isEditMode ng-bind=\"income.amount | ecToDollars\"></span></td><td><span ng-hide=income.deleted ng-click=\"vm.markForDeletion(income, true)\" class=\"text-danger pull-right pointer\"><ec-icon type=trash></ec-icon></span> <span ng-show=income.deleted ng-click=\"vm.markForDeletion(income, false)\" class=\"text-success pull-right pointer\"><ec-icon type=refresh></ec-icon></span></td></tr></tbody><tfoot><tr class=total><th>Total</th><th></th><th>{{ vm.util.total(vm.budget.incomes, 'amount') | ecToDollars }}</th><th></th></tr></tfoot></table><div class=row><div class=col-xs-2 ng-hide=vm.isEditMode><button class=\"btn btn-primary\" ng-click=vm.switchToEditMode()>Make Changes</button></div><div class=col-xs-3 ng-show=vm.isEditMode><button class=\"btn btn-primary\" ng-click=vm.addNewIncome()>Add Income</button></div><div class=col-xs-9><div class=pull-right ng-show=vm.isEditMode><button class=\"btn btn-success\" ng-click=vm.switchToViewMode()>Back to View Mode</button> &nbsp; &nbsp; <button class=\"btn btn-danger\" ng-click=vm.cancelEdit()>Cancel</button></div></div></div></ec-panel></div></div><br>"
  );


  $templateCache.put('app/common/ec-form-field-directive.html',
    "<div class=form-group ng-class=\"{'has-error': vm.error.server }\"><label class=\"col-xs-{{ vm.labelWidth }} control-label\">{{ vm.label }}</label><div class=\"col-xs-{{ vm.fieldWidth }}\"><ng-switch on=vm.type><select ng-switch-when=select name=vm.fieldName ng-model=vm.model ng-required=vm.isRequired ng-options=\"item.id as item.name for item in vm.items\" class=form-control></select><input ng-switch-when=dollars type=number name=vm.fieldName ng-model=vm.model class=form-control ec-as-dollars ng-required=vm.isRequired placeholder=\"{{ vm.placeholder }}\"><input ng-switch-default type=\"{{ vm.type }}\" name=vm.fieldName ng-model=vm.model class=form-control ng-required=vm.isRequired placeholder=\"{{ vm.placeholder }}\"></ng-switch><p class=help-block>{{ vm.error.server }}</p></div></div>"
  );
