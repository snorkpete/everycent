angular.module('everycent').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('app/bank-accounts/form.html',
    "<ec-form-field label=Name label-width=2 name=name error=vm.bankAccountForm.name.$error field-width=6 placeholder=\"Bank Account's name\" type=text ng-required=true ng-model=vm.bankAccount.name></ec-form-field><ec-form-field label=\"Account Type\" label-width=2 name=account_type error=vm.bankAccountForm.account_type.$error field-width=6 placeholder=\"Money Market Fund, Savings, Checking etc\" type=text ng-required=true ng-model=vm.bankAccount.account_type></ec-form-field><ec-form-field label=\"Account Owner\" label-width=2 name=user_id error=vm.bankAccountForm.user_id.$error field-width=6 type=select model-type=user ng-required=true ng-model=vm.bankAccount.user_id></ec-form-field><ec-form-field label=Institution label-width=2 name=institution_id error=vm.bankAccountForm.institution_id.$error field-width=6 type=select model-type=institution ng-required=true ng-model=vm.bankAccount.institution_id></ec-form-field><ec-form-field label=\"Account No\" label-width=2 name=account_no error=vm.bankAccountForm.account_no.$error field-width=6 placeholder=\"Official Account #\" type=text ng-required=true ng-model=vm.bankAccount.account_no></ec-form-field><ec-form-field label=\"Opening Balance\" label-width=2 name=opening_balance error=vm.bankAccountForm.opening_balance.$error field-width=6 placeholder=\"Balance when you started budget\" type=number ng-required=true ng-model=vm.bankAccount.opening_balance></ec-form-field>"
  );


  $templateCache.put('app/bank-accounts/list.html',
    "<div class=row><div class=col-xs-12><h2>Bank Accounts</h2></div></div><div class=row><div class=col-xs-12><table class=\"table table-bordered clear-background\"><thead><tr class=heading><th>Name</th><th>Institution</th><th>Account Type</th><th>Account #</th><th>Owner</th><th>Opening Balance</th><th>Actions</th></tr></thead><tbody><tr ng-repeat=\"bankAccount in vm.bankAccounts\"><td>{{ bankAccount.name }}</td><td>{{ bankAccount.institution.name }}</td><td>{{ bankAccount.account_type }}</td><td>{{ bankAccount.account_no }}</td><td>{{ bankAccount.user.first_name + ' ' + bankAccount.user.last_name }}</td><td>{{ bankAccount.opening_balance }}</td><td><span ng-click=vm.deleteBankAccount(bankAccount) class=\"text-danger pull-right pointer\"><ec-icon type=trash></ec-icon></span> <span ng-click=vm.selectBankAccountForUpdate(bankAccount) class=\"text-warning pull-right pointer\"><ec-icon type=edit></ec-icon>&nbsp; Edit &nbsp; &nbsp; &nbsp;</span></td></tr></tbody></table></div></div><div class=row><div class=col-xs-1 ng-show=\"vm.state.is('bank-accounts')\"><button class=\"btn btn-primary\" ng-click=\"vm.state.goToState('bank-accounts.new')\">Add New Bank Account</button></div><div class=col-xs-1 ng-show=\"vm.state.is('bank-accounts.new')\"><button class=\"btn btn-default\" ng-click=\"vm.state.goToState('bank-accounts')\">Cancel</button></div></div><br><ui-view></ui-view>"
  );
  ...
  $templateCache.put('app/common/ec-form-field-directive.html',
    "<div class=form-group ng-class=\"{'has-error': vm.error.server }\"><label class=\"col-xs-{{ vm.labelWidth }} control-label\">{{ vm.label }}</label><div ng-switch on=vm.type class=\"col-xs-{{ vm.fieldWidth }}\"><select ng-switch-when=select name=vm.fieldName ng-model=vm.model ng-required=vm.isRequired ng-options=\"{{vm.modelType}}.id as {{vm.modelType}}.name for {{vm.modelType}} in vm.lookup.{{vm.modelType}}s\" class=form-control></select><input ng-switch-default type=\"{{ vm.type }}\" name=vm.fieldName ng-model=vm.model class=form-control ng-required=vm.isRequired placeholder=\"{{ vm.placeholder }}\"><p class=help-block>{{ vm.error.server }}</p></div></div>"
  );
