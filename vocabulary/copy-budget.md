...
Duplicate a budget's incomes and allocations to a new budget one month ahead. The primary mechanism for creating new budgets.

## Context

**Replaced recurring allocations.** The original design intended recurring allocations/incomes as templates for new budgets. But every month was nearly identical to the last, so copy budget emerged from practical need and became the standard workflow. The recurring allocations feature was never truly implemented — it's dead code.
