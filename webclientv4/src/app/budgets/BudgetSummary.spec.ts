...
describe('BudgetSummary', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders the spending breakdown bars', () => {
      const wrapper = createWrapper();

      expect(wrapper.find(NEEDS_ROW).exists()).toBe(true);
      expect(wrapper.find(WANTS_ROW).exists()).toBe(true);
      expect(wrapper.find(SAVINGS_ROW).exists()).toBe(true);
    });
  });
  ...
  describe('discretionary amount — couple', () => {
    ...
    // Each person: floor(220000 / 2) = 110000
    const expectedHalf = '1,100.00';

    it('shows discretionary section for couples', () => {
      const wrapper = createWrapper();

      expect(wrapper.find(DISCRETIONARY_SECTION).exists()).toBe(true);
    });

    it('shows wife discretionary with name from settings', () => {
      const wrapper = createWrapper();
      const row = wrapper.find(WIFE_AMOUNT_ROW);

      expect(row.exists()).toBe(true);
      expect(row.text()).toContain("Alice's Discretionary");
      expect(row.text()).toContain(expectedHalf);
    });

    it('shows husband discretionary with name from settings', () => {
      const wrapper = createWrapper();
      const row = wrapper.find(HUSBAND_AMOUNT_ROW);

      expect(row.exists()).toBe(true);
      expect(row.text()).toContain("Bob's Discretionary");
      expect(row.text()).toContain(expectedHalf);
    });
  });
  ...
  describe('discretionary amount — single', () => {
    ...
    it('does not show discretionary section for single', () => {
      const wrapper = createWrapper();

      expect(wrapper.find(DISCRETIONARY_SECTION).exists()).toBe(false);
    });
  });

  describe('spending breakdown — amounts', () => {
  ...
  describe('spending breakdown — percentages', () => {
    // Total income: 600000
    // Needs: 250000 → round(250000/600000 * 100) = 42
    // Savings: 100000 → round(100000/600000 * 100) = 17
    // Wants: 100 - 42 - 17 = 41

    it('shows needs percentage', () => {
      const wrapper = createWrapper();

      expect(wrapper.find(NEEDS_ROW).text()).toContain('42%');
    });

    it('shows wants percentage (remainder after needs and savings)', () => {
      const wrapper = createWrapper();

      expect(wrapper.find(WANTS_ROW).text()).toContain('41%');
    });

    it('shows savings percentage', () => {
      const wrapper = createWrapper();

      expect(wrapper.find(SAVINGS_ROW).text()).toContain('17%');
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('handles zero income without dividing by zero', () => {
      ...
      expect(wrapper.find(NEEDS_ROW).text()).toContain('0%');
      expect(wrapper.find(WANTS_ROW).text()).toContain('0%');
      expect(wrapper.find(SAVINGS_ROW).text()).toContain('0%');
    });
    ...
    it('uses default names when settings fields are not set', () => {
      ...
      expect(wrapper.find(WIFE_AMOUNT_ROW).text()).toContain("Wife's Discretionary");
      expect(wrapper.find(HUSBAND_AMOUNT_ROW).text()).toContain("Husband's Discretionary");
    });
